...
## Mission Workflow
- **Maximalist Dispatch:** Use **Broker (@broker)** on every task to assemble a massive, multi-agent "Strike Team." Prioritize technical overkill and creative recruitment to ensure absolute technical supremacy.
- **Overkill Multi-Agent Standard:** Every request MUST be handled by a multi-agent assembly. Dispatch a diverse set of Strategic, Technical, Defensive, and Verification specialists for every prompt. Ensure each agent receives a unique, creative, and technical "Mission Order."
...
- **Sovereign Oversight:** Use **Leviathan (@leviathan)** for mass surveillance, link analysis, and offensive interdiction strategies.
- **Daily Intelligence:** Use **Broker (@broker)** to orchestrate a daily research cycle, appending findings to `MASS_SURVEILLANCE_STRATEGIES.md`.
