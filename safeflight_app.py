...
def init_db():
    ...
    try:
        ...
        cols = {r[1] for r in cur.execute("PRAGMA table_info(raw_observations)").fetchall()}
        if "reporter" not in cols:
            cur.execute("ALTER TABLE raw_observations ADD COLUMN reporter TEXT")
        cols2 = {r[1] for r in cur.execute("PRAGMA table_info(encounters)").fetchall()}
        if "reporter" not in cols2:
            cur.execute("ALTER TABLE encounters ADD COLUMN reporter TEXT")
    ...
    try:
        ...
        for r in records:
            ...
            cur.execute(
                ...
                (t_ms, lat, lon, speed_mps, heading, reporter, target_id, type, rssi, channel, oui)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    t_ms,
    ...
    try:
        ...
        for e in encounters:
            cur.execute(
                ...
                (encounter_id, target_id, reporter, start_t, end_t, count_hits, rssi_max, t_at_rssi_max, peak_lat, peak_lon, confidence)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    e.get("encounter_id"),
...
def summarize_encounter(recs):
    ...
    return {
        ...
        "reporter": max_rec.get("reporter", "unknown"),
...
def rebuild_map(records, center=None, current_mph=None, map_mode="car"):
    ...
    vehicle_symbol = "🏍️" if map_mode == "bike" else "🚗"
    folium.Marker(
        [center[0], center[1]],
        icon=folium.DivIcon(html=f"<div style='font-size:20px'>{vehicle_symbol}</div>"),
        tooltip=f"Reporter vehicle ({map_mode})"
    ).add_to(m)
    ...
    for e in encounters:
        ...
        hover_preview = (
            f"{r['name'] or '(unnamed)'} | {r['device_kind']} | {r.get('reporter', 'unknown')} | {r['t_at_rssi_max']}"
        )
        popup = (
            ...
            f"<b>Reporter:</b> {r.get('reporter', 'unknown')}<br>"
            f"<b>Signal:</b> {sig}<br>"
...
class App:
    def __init__(self, root):
        ...
        self.map_mode = tk.StringVar(value="car")
    ...
    def load_settings(self):
        ...
        try:
            ...
            self.map_mode.set(d.get("map_mode", "car"))
        except Exception:
            pass
    ...
    def refresh_map(self):
        ...
        try:
            ...
            mph = None
        rebuild_map(recs, center=self.map_center, current_mph=mph, map_mode=self.map_mode.get())
    ...
    def open_settings_dialog(self):
        ...
        ttk.Label(frm, text="Map display mode").pack(anchor="w", pady=(0, 6))
        mode = ttk.Combobox(frm, textvariable=self.map_mode, values=["car", "bike"], state="readonly")
        mode.pack(fill="x")
        ttk.Button(
            frm,
            text="Save",
        ...
        while self.running:
            try:
                ...
                for r in raw:
                    ...
                    rr["reporter"] = self.user_name.get().replace("User: ", "", 1).strip() or "unknown"
                    batch.append(rr)
